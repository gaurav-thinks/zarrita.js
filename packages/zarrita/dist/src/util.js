...
export function get_ctr(data_type) {
    ...
    if (data_type && typeof data_type === "object") {
        ...
        if (dt.name === "null_terminated_utf8" && dt.configuration?.length_bytes != null) {
            return UnicodeStringArray.bind(null, dt.configuration.length_bytes);
        }
        // v3 fixed-length / null-terminated raw bytes (S<n>)
        if (dt.name === "null_terminated_bytes" && dt.configuration?.length_bytes != null) {
            return ByteStringArray.bind(null, dt.configuration.length_bytes);
        }
        ...
        if (isV3StringObject(dt)) {
            ...
            if (name === "fixed_length_utf32") {
                ...
                return UnicodeStringArray.bind(null, codepoints);
            }
    ...
    if (match) {
        ...
        if (kind === "U") {
            return UnicodeStringArray.bind(null, length);
        }
        else {
            return ByteStringArray.bind(null, length);
        }
    }
    const ctr = {
        int8: Int8Array,
